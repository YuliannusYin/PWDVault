...
```
%APPDATA%\PwdVault\
├── vault.db   # SQLite 数据库（password 字段为 AES-256-GCM 密文 + iv + tag）
└── vault.meta # Argon2id salt + 加密的 master_key
```

新版使用 Argon2id 从主密码派生 KEK，再加密随机生成的 32 字节 master_key；
master_key 用于 AES-256-GCM 加密每条记录的 password 字段。详见
...
```powershell
pwdvault-migrate.exe --master-password=YourNewMasterPassword
```

工具会自动从默认位置读取旧库与旧密钥，并写入新库到默认位置。
若不传 `--master-password`，工具会在控制台交互式提示输入两次主密码（不回显）。
...
| `--master-password=<pw>`      | 新主密码（不传则交互式输入）                                 | —（交互式输入）                              |
...
3. **正式迁移**：
   ...
   ```
   按提示输入新的主密码（建议使用强密码）。
4. **验证新库**：启动 `pwdvault-ui.exe`，用刚设置的主密码登录，确认所有条目
   都能正确显示且密码能解密。
...
1. **参数解析**：解析命令行，确定旧库、旧密钥、新库路径与主密码来源。
2. **存在性检查**：确认旧库与旧密钥文件存在；若新库的 `vault.meta` 已存在
   则报错退出（避免覆盖已初始化的库）。
...
5. **初始化新库 master key**：
   ...
   - 构造 `MasterKeyStore`，调用 `initialize(master_password)`：
     生成 16 字节 salt、用 Argon2id 派生 KEK、生成 32 字节 master_key、
     用 AES-256-GCM 加密 master_key 后写入 `vault.meta`。
...
7. **逐条迁移**（在单个 SQLite 事务内）：
   ...
   - 用新版 `CryptoEngine`（以 master_key 构造）对明文密码执行 AES-256-GCM
     加密，得到 `[IV(12) || ciphertext || tag(16)]`。
   - 拆分填充到 `entry.password / iv / tag`，调用 `storage.add_entry()` 写入。
8. **提交事务**：所有成功条目一次性提交；若全部失败则回滚。
9. **打印统计**：输出总数、成功数、失败数，以及旧文件位置提示。
...
1. 启动 `pwdvault-ui.exe`，用迁移时设置的主密码登录。
...
### 6.6 报错："Master password must not be empty"

未提供主密码或两次输入不一致。重新运行工具并输入合法主密码。

### 6.7 报错："begin_transaction / commit_transaction failed"
...
- 迁移过程中，旧 Fernet 密钥与主密码在内存中以明文短暂存在；工具退出时会
  通过 `sodium_memzero` 清零相关缓冲区。
