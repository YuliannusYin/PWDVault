...
//
// 设计要点：
//   - 主密钥（32 字节）在构造时拷贝入对象，析构时用 sodium_memzero 清零。
//   - OpenSSL 资源用 unique_ptr + 自定义 deleter 进行 RAII 管理。
//   - 不可拷贝、不可移动（避免主密钥在堆中被多次复制散落）。
//
// 依赖：
...
public:
    /// 构造加密引擎。
    /// \param master_key 主密钥（必须为 32 字节，用于 AES-256）；非 32 字节时
    ///                   对象仍可构造，但 encrypt/decrypt 会返回 CryptoError
    explicit CryptoEngine(core::ByteSpan master_key);
